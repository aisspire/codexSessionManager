...
- 批量编辑标题前缀、提供方和项目路径。
- 单个或批量归档、设为活动、置顶、删除到工具回收区，并支持单个会话上下文压缩。
- 收藏重点会话，并用收藏范围快速过滤。
...
- 归档、活动、删除、压缩上下文、数据库修复等写入操作会检测 Codex 是否正在运行；检测到同一份本地数据可能被占用时会拒绝写入。

建议在执行大批量整理、数据库修复、恢复或上下文压缩前关闭正在使用同一份数据的 Codex。对于特别重要的数据，也可以先额外复制 `.codex` 中的关键文件。
